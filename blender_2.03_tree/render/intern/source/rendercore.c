 ...
 *  maart-april 95
 * 
 * Version: $Id: rendercore.c,v 1.7 2000/09/21 13:46:23 nzc Exp $
 */
...
#include "texture.h" /* for render_realtime_texture */
#include "renderfg.h"
...
/*
  There are some problems here on Linux (gcc): tests on cp and cp1
  sometimes do not evaluate correctly because of limited width of the
  variables. I have replaced them with something that makes more
  sense. Maybe widening the test vars is better?
 */
...
void zbufshadeDA()	/* Delta Accum Pixel Struct */
{
	...
	if( (R.r.mode & R_EDGE) && RE_test_break()==0) {
		rt= R.rectot;
		rp= R.rectaccu;
...
void zbufshade()
{
	...
	/* if((R.flag & R_HALO) && RE_test_break()==0) halovert(); */
