 ...
 *  maart 95
 * 
 * Version: $Id: texture.c,v 1.5 2000/09/21 13:46:23 nzc Exp $
 */
...
void open_plugin_tex(PluginTex *pit)
{
	...
	version= (int (*)()) dlsym(pit->handle, "plugin_tex_getversion");
	...
	if (pit->version==2 || pit->version==3) {
		...
		info_func= (int (*)(PluginInfo *))dlsym(pit->handle, "plugin_getinfo");
...
void do_2d_mapping(MTex *mtex, float *t, float *dxt, float *dyt)
{
	...
	if(R.osa==0) {
		...
		if(wrap==MTEX_FLAT) {
			...
			else if(R.vlr==NULL || mtex->texco==TEXCO_GLOB) cubemap_glob(mtex, t[0], t[1], t[2], &fx, &fy);
		...
		if(wrap==MTEX_FLAT) {
			...
			else if (R.vlr==NULL || mtex->texco==TEXCO_GLOB) proj = cubemap_glob(mtex, t[0], t[1], t[2], &fx, &fy);
		...
		if(la->mtex[tex_nr]) {
			...
			if(mtex->texco==TEXCO_OBJECT) {
				...
				if(R.osatex) {
					VECCOPY(dxt, O.dxlv);
					VECCOPY(dyt, O.dylv);
					/* need some matrix conversion here? */
