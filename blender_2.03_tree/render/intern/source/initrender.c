...
/* Global includes */
#include "blender.h"
#include "graphics.h"	/* voor drawing stars, evil! */
#include "edit.h"
#include "render.h"
#include "file.h"
#include "ipo.h"
...
/* ~~~~~~~~~~~~~~~~~ADD~~~~~~~~~~~~~~~~~~~~~ */
/* These functions not only add data structures, they also act as lookup    */
/* functions! They need to be split.                                        */
/* ------------------------------------------------------------------------ */
...
void make_render_halos(Object *ob, Mesh *me, Material *ma, float *extverts)
{
	...
	for(a=start; a<end; a++, mvert++) {
		...
		if(ok) {
			...
			else har= inithalo(ma, vec, 0, mvert->co, hasize, 0);
			if(har) har->lay= ob->lay;
		}
		if(orco) orco+= 3;
		ma->ren->seed1++;
...
int mesh_test_flipnorm(Object *ob, MFace *mface, VlakRen *vlr, float imat[][3])
{
	DispList *dl;
	Mesh *me= ob->data;
...
void init_render_s_mesh(Object *ob)
{
	...
	int a, b, flipnorm= -1,  need_orco=0, startvlak, startvert, p1, p2, p3, p4;
...
void roteerscene()
{
   extern int slurph_opt;	/* key.c */
	...
	if(R.wrld.mode & WO_STARS) RE_make_stars(0);
...
void RE_initrender()
{
	...
	if(R.r.scemode & R_DOSEQ) {
		...
		if(RE_test_break()==0) do_render_seq();

		/* displayen */
		if(R.rectot) RE_render_display(0, R.recty-1);
	}
...
void RE_animrender()
{
	...
	for(CFRA=SFRA; CFRA<=EFRA; CFRA++) {
		...
		if(RE_test_break()==0) {
			...
			if(R.r.imtype==R_MOVIE) {
				...
				schrijfplaatje(name);
				if(RE_test_break()==0) printf("Saved: %s", name);
			}
	...
	if(G.background==0) RE_exit_render_stuff();	/* voor end_camera_net */
