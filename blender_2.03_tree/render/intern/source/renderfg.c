 ...
 * 
 * Version: $Id: renderfg.c,v 1.4 2000/09/08 16:41:21 nzc Exp $
 */
...
void RE_render_display(int starty, int endy)
{
	...
	if(R.displaymode==R_DISPLAYWIN) {
		if(R.win) {
         ...
         /* set valid raster position */
			glRasterPos2f(-0.01,  -0.01+(float)starty);
         /* stream data starting at rect to the current raster position  */
         /* dump a total of R.rectx pixels per line, endy-starty+1 lines */
			glDrawPixels(R.rectx, endy-starty+1, GL_RGBA, GL_UNSIGNED_BYTE, rect);
...
{
	if(R.win) RE_redraw_render_win(R.win);
	else {
		;
	}
}

void RE_toggle_render_display()
{
	ScrArea *sa;
	View3D *vd;
	int ok= 1;
