 ...
 *  may 1999
 * 
 * Version: $Id: envmap.c,v 1.5 2000/09/14 10:46:50 nzc Exp $
 */
...
void env_rotate_scene(float mat[][4], int mode)
{
	...
	for(a=0; a<R.totlamp; a++) {
		lar= R.la[a];
		Mat3CpyMat3(smat, lar->imat); /* smat is 4by4*/
		Mat3MulMat3(lar->imat, smat, imat); /* redef may be strange */
...
void render_envmap(EnvMap *env)
{
	...
	if(RE_test_break()) RE_free_envmapdata(env);
	else {
