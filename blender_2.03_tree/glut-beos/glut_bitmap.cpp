...
void APIENTRY 
myglScissor(int a, int b, int c, int d)
{
	BRegion clipper;
	BRect r(a,b,c,d);
	...
	printf("%x \n", gState.currentWindow);
	if(gState.currentWindow) {
		//gState.currentWindow->LockGL();
		printf("ervoor\n");	
		//gState.currentWindow->ConstrainClippingRegion(NULL);
		printf("dinges\n");
		gState.currentWindow->ConstrainClippingRegion(&clipper);
