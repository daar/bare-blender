...
/* Version: $Id: Button.c,v 1.8 2000/07/21 09:05:21 nzc Exp $ */
...
void graphics_to_window(float *x, float *y)	/* voor rectwrite b.v. */
{
	float gx, gy;
	int sx, sy;
...
short GetButVal(struct But *but, float *fvalue, int *lvalue)
{
	...
	if (format==FORMAT_INT) *fvalue= (float)*lvalue;
	else if (format==FORMAT_FLO) *lvalue= (int) *fvalue;
...
void ScrollDrawFunc(x1,y1,x2,y2,s1,s2,sel,col,oud)
short x1,y1,x2,y2,s1,s2,sel,col,*oud;
{
	struct ButCol *bc;
	...
	cpack(0x505050);
	glRecti(x1,  y1,  x2,  y2);
	EmbossBox2(x1-2, y1-2, x2+2, y2+2, 1, 0x404040, 0xA0A0A0);
	...
	sel? cpack(bc->border1): cpack(bc->paper_sel);
	glRecti(bx1, by1, bx2, by2);
	EmbossBox2(bx1-2, by1-2, bx2+2, by2+2, sel, 0x404040, 0xA0A0A0);
}
...
void DrawBut2(struct But *but, short sel)
{
	...
	case SCROLL:
		{
			float f1, pos, buth, sliderh, toth;
			short s1, s2;
			float value_len= but->max - but->min;
			...
			sliderh= (buth/toth)*buth;
			if (sliderh<10.0) sliderh= 10.0;

			pos= (value - but->min)/value_len;

			s2= but->y2 - pos*(buth-sliderh);
			s1= s2 - sliderh;

			CLAMP(s1, but->y1, but->y2);
			CLAMP(s2, but->y1, but->y2);

			ScrollDrawFunc(but->x1,but->y1,but->x2,but->y2,s1,s2,sel,but->col,but->rt);
...
static void do_sliderbut(struct But *but)
{
	...
	int a, sx, h, temp, pos=0, lvalue, redraw;
	short mval[2], qual;
	...
	while (get_mbut() & L_MOUSE) {
		...
		if (GetButVal(but, &value, &lvalue)==FORMAT_INT)
			redraw= (temp != lvalue);
		else
			redraw= (tempf != value);
...
static void do_scrollbut(struct But *but)
{
	float f, fstart, tempf, deler, value;
	int a, sy, h, temp, pos=0, lvalue, redraw;
	short mval[2], qual;
	...
	deler= (but->y1-but->y2-h);
	
	while (get_mbut() & L_MOUSE) {
		qual= get_qual();
		ButtonsGetmouse(mval);
		f= (float)(mval[1]-sy)/deler +fstart;
		if(qual & LR_CTRLKEY) {
			if(qual & LR_SHIFTKEY) f= ffloor(f*100.0)/100.0;
			else f= ffloor(f*10.0)/10.0;
		} else if (qual & LR_SHIFTKEY) {
	...
	if(temp!=32767 && pos==0) {  /* plus 1 of min 1 */
		...
		f= (float)(mval[1]-but->y2)/(but->y1-but->y2-h);
				...
				if(mval[1]>=b->y1-temp && mval[1]<=b->y2+temp) {
					if(b->lock) {
						...
						case MENU:
							DrawBut(b, 1);
							retval= domenubut(b);
							usleep(10);		/* patch for slow x drawing SGI */
							DrawBut(b, 0);
							break;
						case ICONROW:
