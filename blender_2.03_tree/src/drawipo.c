 ...
 * 
 * 
 * Version: $Id: drawipo.c,v 1.12 2000/09/10 18:42:07 ton Exp $
 */
...
void draw_ipobuts()
{
	...
	sprintf(naam, "ipowin %d", curarea->win);
	block= uiNewBlock(&curarea->uiblocks, naam, UI_EMBOSSN, UI_HELV, 0x808080, curarea->win);
	block->col= BUTRUST;
	...
	for(a=0; a<G.sipo->totipo; a++, ei++, y-=IPOBUTY) {
		...
		but= uiDefBut(block, TOG|INT|BIT|a, a+1, ei->name,  G.v2d->mask.xmax+18, y, IPOBUTX-15, IPOBUTY-1, &(G.sipo->rowbut), 0, 0, 0, 0, "");
		but->flag |= UI_TEXT_LEFT;

		if(ei->icu) {
			cpack(ei->col);

			glRects(G.v2d->mask.xmax+8,  y+2,  G.v2d->mask.xmax+15, y+IPOBUTY-2);
			sel= ei->flag & (IPO_SELECT + IPO_EDIT);

			uiEmboss((float)(G.v2d->mask.xmax+8), (float)(y+2), (float)(G.v2d->mask.xmax+15), (float)(y+IPOBUTY-2), sel);
		}
