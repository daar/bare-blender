 ...
 *  
 * 
 * Version: $Id: sequence.c,v 1.5 2000/08/28 13:21:03 nzc Exp $
 */
...
void open_plugin_seq(PluginSeq *pis, char *seqname)
{
	...
	version= (int (*)())dlsym(pis->handle, "plugin_seq_getversion");
	...
	if (pis->version==2 || pis->version==3) {
		...
		info_func= (int (*)(PluginInfo *))dlsym(pis->handle, "plugin_getinfo");
			...
			if(se) {
				if(seq->type == SEQ_META) {
					...
					if(seq->type==SEQ_IMAGE) {
						...
						if(G.background) RE_initrender();
						else RE_do_renderfg_seq();		/* renderfg.c */
