 ...
 *  
 * 
 * Version: $Id: editmesh.c,v 1.16 2000/09/24 20:22:10 ton Exp $
 */
...
int vergvlak(const void *v1, const void *v2)
{
...
EditVlak *addvlaklist(EditVert *v1, EditVert *v2, EditVert *v3, EditVert *v4, EditVlak *example)
{
	...
	if(example) {
		...
		evl->col[0]= evl->col[1]= evl->col[2]= evl->col[3]= 0x77777777;
	}
...
void load_editMesh()
{
	...
	if(actkey && me->key->refkey!=actkey) {
		...
		if(me->mcol && me->totface) {
			...
			mc=mcn= mallocN(4*sizeof(int)*me->totface, "mcol");
			evl= G.edvl.first;
			while(evl) {
	...
	*/
	...
	while(evl) {
		...
		if(evl->f & 1) {
			...
			evln= addvlaklist(v1, v2, v3, v4, evl);
...
void uv_half(float *uv, float *uv1, float *uv2)
{
...
void uv_quart(float *uv, float *uv1)
{
...
float *set_correct_uv(EditVert *eve,  EditVlak **evla)
{
...
void join_mesh()
{
	...
	if(me->tface) tface= tfacemain= callocN(totface*4*sizeof(TFace), "joinmesh4");
	else tfacemain= 0;
...
int verg_mface(const void *v1, const void *v2)
{
	const MFace *x1=v1, *x2=v2;
	MVert *ve1, *ve2;
...
void sort_faces()
{
	...
	qsort(me->mface, me->totface, sizeof(MFace), verg_mface);
