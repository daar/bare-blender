 ...
 *
 *				Paul Haeberli - 1990
 * Version: $Id: psfont.c,v 1.4 2000/07/21 09:05:28 nzc Exp $
 */
...
void makeobjfont(int savesplines);
...
char *my_subrs[MAXSUBRS];
unsigned int my_sublen[MAXSUBRS];
char *my_chars[MAXCHARS];
...
static void applytocharverts(objfnt *fnt, int c, int (*func)())
{
	int index;
	chardesc *cd;
	short *sptr;
...
static void calccharbboxes(objfnt *fnt)
{
	int c;
	chardesc *cd;
...
char * newfgets(char * s, int n, PackedFile * pf){
	int read = 0;
	int c;
	char * p;
...
int readfontmatrix(PackedFile * pf, float mat[2][2])
{
	char *cptr;
	float a, b, c, d, e, f;
...
int decodetype1(PackedFile * pf, char *outname)
{
	FILE *outf;
	char *hptr, *bptr;
	...
	/* allocate buffers */
	totlen = pf->size;
	hexdat = (char *)mallocN(totlen, "hexdat");
	bindat = (char *)mallocN(totlen, "bindat");
...
objfnt * loadpostscriptfont(PackedFile * pf)
{
...
static void initretstack()
{
	retsp = 0;
}

static void retpush(val)
int val;
{
	retstack[retsp] = val;
	retsp++;
...
static void poly_beginchar()
{
	npnts = 0;
	nloops = 0;
}

static void poly_endchar()
{
	if(nloops == 0)
...
static void spline_beginchar()
{
	sp_npnts = 0;
	sp_nloops = 0;
}

static void spline_endchar()
{
	if(sp_nloops == 0)
