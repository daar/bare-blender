 ...
 *  maart 95
 * 
 * Version: $Id: image.c,v 1.10 2000/08/28 12:23:45 nzc Exp $
 */
...
void load_image(Image * ima, int flags)
{
	...
	if (ima->ibuf == NULL) {
		...
		if (ima->packedfile) {
			ima->ibuf = ibImageFromMemory((int *) ima->packedfile->data, ima->packedfile->size, flags);
		} else {
			strcpy(name, ima->name);
			convertstringcode(name);

			ima->ibuf = loadiffname(name , flags);
		}
	}
}
...
void ima_ibuf_is_nul(Tex *tex)
{
	...
	if(tex->imaflag & TEX_ANIM5) {
		...
		// for performance (loadiffname uses mmap) we don't do this by default
