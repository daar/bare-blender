...
void shadeDispList(Object *ob)
{
	...
	MetaBall *mb;
	extern Material defmaterial;	/* initrender.c */
	float *orco, imat[3][3], tmat[4][4], mat[4][4], vec[3], tvec[3], xn, yn, zn;
...
float DistToLine2D(short *v1, short *v2, short *v3)   /* met formule van Hesse :GEEN LIJNSTUK! */
{
	float a[2],deler;
...
float ComputeMaxShpError(Sample *samp, int first, int last, int *splitPoint)
...
void FitPoly(Sample *samp, int first, int last, float shperr, ListBase *seglist)
    /* Samp: Array of digitized points */
