 ...
 *  maart 95
 *  
 * Version: $Id: curve.c,v 1.4 2000/07/21 09:05:23 nzc Exp $
 */
...
Nurb *duplicateNurb(Nurb *nu)
{
	...
	newnu= mallocstructN(Nurb, 1, "duplicateNurb");
	if(newnu==0) return 0;
	memcpy(newnu, nu, sizeof(Nurb));

	if(nu->bezt) {
		newnu->bezt= mallocstructN(BezTriple, nu->pntsu, "duplicateNurb2");
		memcpy(newnu->bezt, nu->bezt, nu->pntsu*sizeof(BezTriple));
	}
	else {
		len= nu->pntsu*nu->pntsv;
		newnu->bp= mallocstructN(BPoint, len, "duplicateNurb3");
		...
		if(nu->knotsu) {
			len= KNOTSU(nu);
			if(len) {
				newnu->knotsu= mallocN(len*sizeof(float), "duplicateNurb4");
		...
		if(nu->pntsv>1 && nu->knotsv) {
			len= KNOTSV(nu);
			if(len) {
				newnu->knotsv= mallocN(len*sizeof(float), "duplicateNurb5");
